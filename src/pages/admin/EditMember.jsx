...
import { EditMemberDetails, GetMemberInfoById, GetAllScrumTeam, GetTeamDetailsByMemberId } from '../../apis/userservice';
...
const EditMember = () => {
    ...
    useEffect(() => {
        ...
        if (!loadStatus) {
            loadMemberInfo();
            loadTeamDetailsByMemberId();
            setLoadStatus(true)
        }
        // if(formValues.role !== "Admin" && formValues.scrumTeamId == ""){
    ...
    const loadTeamDetailsByMemberId = () => {
        GetTeamDetailsByMemberId(memberId).then((res) => {
            // console.log(res.teamId.teamId);
            formValues['scrumTeamId'] = res.teamId.teamId;
        }).catch((err) => {
    ...
    const loadMemberInfo = () => {
        GetMemberInfoById(memberId).then((res) => {
            formValues['firstName'] = res.firstName;
            formValues['lastName'] = res.lastName;
            formValues['dob'] = res.dob;
            formValues['mailId'] = res.mailId;
            formValues['role'] = res.role;
            // setFormValues({ firstName: res.firstName, lastName: res.lastName, dob: res.dob, mailId: res.mailId, role: res.role });
        }).catch((err) => {
    ...
    const onSubmit = async () => {
        ...
        EditMemberDetails(reqData).then((res) => {
            setIsToastOpen(true);
            setUpdate(false);
